...
export async function getToolManifest(
  ...
  try {
    ...
    console.error(
      `[ToolingEngine] Failed to load manifest for tool: ${toolName}`,
      error
    );
    return null;
  }
...
export default async function DynamicLoader({
  toolName,
  themeName,
  fallback,
}: DynamicLoaderProps) {
  ...
  try {
    ...
    if (!loader) {
      throw new Error(
        `Tool "${toolName}" is not registered in TOOL_REGISTRY. Please add it to DynamicLoader.tsx.`
      );
    }
    ...
    return (
      <div
        className={`${scopeClassName} ${skinClassName} flex h-full w-full flex-col`}
      >
        <Suspense
          fallback={
            fallback || <div className="p-4">Loading tool component...</div>
          }
        >
    ...
    console.error(
      `[ToolingEngine] Failed to load tool component: ${toolName}`,
      error
    );
    return (
      <div className="rounded-md border border-red-500 bg-red-50 p-4 text-red-700">
        ...
        <p>
          Failed to render tool &quot;{toolName}&quot;. {String(error)}
        </p>
      </div>
    );
  }
